...
exports.getAnalytics = async (req, res) => {
  const [totalPatients, totalForms, totalDrafts, formsByType, recentSubmissions, genderDist, monthlyTrend] = await Promise.all([
    Patient.countDocuments(),
    FormSubmission.countDocuments(),
    Draft.countDocuments({ isSubmitted: false }),
    FormSubmission.aggregate([{ $group: { _id: '$formType', count: { $sum: 1 } } }]),
  ...
  res.json({
    ...
    data: {
      ...
      formsByType: formsByType.map(f => ({ name: f._id, value: f.count })),
      recentSubmissions,
      genderDist: genderDist.map(g => ({ name: g._id||'Unknown', value: g.count })),
      monthlyTrend: monthlyTrend.map(d => ({ month: months[d._id.month-1]+' '+d._id.year, submissions: d.count }))
    }
  });
};
